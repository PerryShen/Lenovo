    ...
    #do
		...
		echo "play $file"
		  ...
		  #	echo "nlu_reply=$nlu_reply"	
			echo "The songName... ..."
		    songName=`$ssh_cmd$ip "cat /data/messages | grep L_MSG_CB_NOTIFY_AUDIO_STARTPLAY | sed 's/.*\"songName\"://g' | cut -d , -f 1 | sed 's/[\t]*//g' | awk 'END{print}'"`
			echo "songName=$songName"
			echo "The songId... ..."
		    songId=`$ssh_cmd$ip "cat /data/messages | grep L_MSG_CB_NOTIFY_AUDIO_STARTPLAY | sed 's/.*\"songId\"://g' | cut -d , -f 1 | sed 's/[\t]*//g' | awk 'END{print}'"`
			echo "songId=$songId"
			echo "The artistName... ..."
		    artistName=`$ssh_cmd$ip "cat /data/messages | grep L_MSG_CB_NOTIFY_AUDIO_STARTPLAY | sed 's/.*\"artistName\"://g' | cut -d , -f 1 | sed 's/[\t]*//g' | awk 'END{print}'"`
			echo "artistName=$artistName"
			echo "The artistId... ..."
		    artistId=`$ssh_cmd$ip "cat /data/messages | grep L_MSG_CB_NOTIFY_AUDIO_STARTPLAY | sed 's/.*\"artistId\"://g' | cut -d , -f 1 | sed 's/[\t]*//g' | awk 'END{print}'"`
			echo "artistId=$artistId"
			echo "The speaker's reply... ..."
		    reply=`$ssh_cmd$ip "cat /data/messages | grep L_MSG_CB_NOTIFY_AUDIO_STARTPLAY | sed 's/.*\"reply\"://g' | cut -d , -f 1 | sed 's/[\t]*//g' | awk 'END{print}'"`
			echo "reply=$reply"
		        echo "Recording the audio file name we triggered into the native log for the later comparision... ..."
                    asr_org=`echo $file | cut -d . -f 2`
			...
			echo "asr_org=$asr_org"                    
                    ...
                    echo "songName=$songName" >> ${path}${time}_${log_file}
                    echo "songId=$songId" >> ${path}${time}_${log_file}
		    echo "artistName=$artistName" >> ${path}${time}_${log_file}
		    echo "artistId=$artistId" >> ${path}${time}_${log_file}

                    echo "reply=$reply" >> ${path}${time}_${log_file}
